...
var Favorites = /** @class */function () {
    ...
    Favorites.prototype.updateFavoritesCount = function () {
        ...
        $('[data-favorites-count]').attr('data-favorites-count', favoritesCount).text(favoritesCount);
    };
...
jQuery(document).ready(function ($) {
    $('.catalog-accordion__toogler[data-action="toggle-collapse"]').on('click', function (e) {
        ...
        accordionItemContent.collapse('toggle').on('shown.bs.collapse', function () {
...
jQuery(document).ready(function ($) {
    ...
    var $noUiSliderElement = $('[noui-slider]'),
    ...
    catalogFilterElement.on('click', '[data-action="append-row"]', function (e) {
        e.preventDefault();
        e.stopPropagation();
        var $this = $(this),
        ...
        rowClone.addClass('js-appended');
        rowClone.find('.catalog-filter__multiple-toggle').attr('data-action', 'remove-row');
        rowClone.find('label').addClass('d-none');
    ...
    function selectUniqueMultipleOption(controlGroupElement, clonedControlElement) {
        ...
        controlGroupElement.find('option:selected').each(function (index, selectedOption) {
        ...
        clonedControlElement.find('option[selected]').removeAttr('selected');
        clonedControlElement.find('option:eq(' + uniqueIndex + ')').attr('selected', 'selected');
    }
    // Set unique control names for cloned elements
    function updateMultipleControlNames(controlGroupElement) {
        controlGroupElement.find('.catalog-filter__multiple-control').each(function (index, controlElement) {
...
jQuery(document).ready(function ($) {
    ...
    function transfromTabsToAccordion() {
        ...
        if (window.outerWidth < 768 && !isInitialized) {
            catalogTabsElement.addClass(accordionClassName).find('.tab-pane').each(function () {
                var paneElement = $(this),
                    paneNavItem = paneElement.attr('id'),
                    paneNavItemElement = $('[href="#' + paneNavItem + '"]');
                paneElement.removeClass('active').removeClass('fade').addClass('collapse');
                paneNavItemElement.removeClass('active').after(paneElement).on('click', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    paneElement.toggleClass('active').collapse('toggle');
                });
            });
        }
    }
...
jQuery(document).ready(function ($) {
    ...
    function updateAdaptivityFooter() {
        if (window.innerWidth < 560) {
            ...
            pageFooterNavTitles.addClass('collapsed').on('click', function (e) {
                $(this).toggleClass('collapsed').parent().find('.page-footer__nav').collapse('toggle');
            });
        } else {
            pageFooterNavs.removeClass('collapse');
            pageFooterNavTitles.removeClass('collapsed').off('click');
        }
    }
...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        paginationElement.find('.page-item.active').removeClass('active');
        paginationElement.find('.page-link[href*="page=' + activePage + '"]').parent().addClass('active');
    });
})(jQuery);
jQuery(document).ready(function ($) {
